-- Admin Verification Schema Changes (Fixed Version)
...
-- Note: Using TEXT for entity_id to handle both UUID and BIGINT
...
SELECT 
    ...
    plant_status_id::text as entity_id,
    verification_status,
    submitted_at,
    verified_at,
    admin_notes,
...
-- This function handles both UUID (farmer_detail) and BIGINT (other tables)
CREATE OR REPLACE FUNCTION update_verification_status(
    p_table_name TEXT,
    p_entity_id TEXT,  -- Using TEXT to handle both UUID and BIGINT
    p_status VARCHAR(20),
...
DECLARE
    ...
    v_old_data JSONB;
    v_new_data JSONB;
    v_entity_id_typed TEXT;
BEGIN
    -- Handle different ID types based on table
    IF p_table_name = 'farmer_detail' THEN
        v_entity_id_typed := p_entity_id::uuid::text;
    ELSE
        v_entity_id_typed := p_entity_id::bigint::text;
    END IF;
    ...
    INTO v_old_data
    USING CASE 
        WHEN p_table_name = 'farmer_detail' THEN p_entity_id::uuid
        ELSE p_entity_id::bigint
    END;
    ...
    EXECUTE v_sql USING p_status, p_admin_notes, p_admin_user_id, CASE 
        WHEN p_table_name = 'farmer_detail' THEN p_entity_id::uuid
        ELSE p_entity_id::bigint
    END;
