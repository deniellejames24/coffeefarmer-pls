...
Based on Philippine National Standards (PNS)
...
# =====================================
    ...
    with tab2:
        ...
        with col1:
            # Temperature impact - remove trendline
            ...
            # Soil pH impact - remove trendline
        ...
        with col2:
            # Rainfall impact - remove trendline
            ...
            # Soil moisture impact - remove trendline
...
# =====================================
    ...
    with col1:
        ...
        bean_screen = st.slider("Bean Screen Size (mm)", min_value=4.0, max_value=7.5, value=6.5, step=0.1,
    ...
    with col2:
        ...
        # Calculate derived indices with elevation
        # Elevation suitability for Robusta (600-1200 masl optimal)
        elevation_optimal_center = 900  # midpoint of 600-1200
        elevation_optimal_range = 300  # half of the range
        elevation_score = 1 - abs(elevation - elevation_optimal_center) / elevation_optimal_range
    ...
    if st.button("🔮 Predict Coffee Grade", key='predict_grade', type='primary'):
        with st.spinner("Calculating predictions..."):
            # Determine grade based on CQI/UCDA standards
            ...
            with col1:
                ...
                st.metric(
                    ...
                    delta=f"Cupping: {cupping_score:.1f}"
                )
            ...
            requirements = pd.DataFrame({
                ...
                'Cupping Score': ['≥80', '≥80', '<80', f"{cupping_score:.1f}"]
            })
            
            st.dataframe(requirements, use_container_width=True, hide_index=True)
            st.caption("*Combined primary and secondary defects")
            ...
            with col2:
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    value=cupping_score,
                    domain={'x': [0, 1], 'y': [0, 1]},
                    title={'text': "Estimated Cupping Score"},
                    gauge={
                        'axis': {'range': [0, 100]},
                        'bar': {'color': "#2c5f2d"},
                        'steps': [
                            {'range': [0, 70], 'color': "#ffcccb"},
                            {'range': [70, 80], 'color': "#ffffcc"},
                            {'range': [80, 85], 'color': "#add8e6"},
            ...
            if bean_screen < 6.5:
                st.info("📏 **Bean size below optimal** - Focus on:")
                st.markdown("- Improve plant nutrition (complete fertilizer 14-14-14)")
                st.markdown("- Ensure adequate water during cherry development")
                st.markdown("- Proper spacing (3m x 2m) for better growth")
...
# =====================================
    ...
    with col2:
        ...
        st.markdown("#### 🐛 Pest Management Program")
        ...
        with st.spinner("Calculating yield and grade forecasts..."):
            ...
            with col1:
                ...
                if pest_management_frequency < 3:
                    st.info("🐛 **Improve pest management** to reduce losses")
